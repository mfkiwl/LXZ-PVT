...
* 该程序读取后不会影响指针的指向，即读完后buff指针依旧指向1，而非6 or 7
...
extern void OutLog(char* stm, time_t EndTime, const char* logfilepath, prcopt_t opt, int flag)
{
    ...
    // 尝试打开文件，如果文件存在，打开并附加内容；如果文件不存在，则创建文件
    LogFile = fopen(FilePath, "a");  // 'a' 模式：如果文件存在，追加内容；如果不存在，创建新文件
    if (LogFile == NULL)
    {
        perror("Error opening file");  // 打开文件失败，输出错误信息
        return;  // 退出函数
    }
    ...
    if (opt.ionoopt == IONOOPT_IFLC)
    {
        ...
        strcat(LocalLog, "系统双频伪距定位|");
        strcat(LocalLog, "北斗/GNSS基本导航定位|");
        strcat(LocalLog, "双频定位解算|");
    }
    else
    {
        if (opt.navsys & SYS_GPS)
        ...
        strcat(LocalLog, "系统单频伪距定位|");
        strcat(LocalLog, "北斗/GNSS基本导航定位|");
        strcat(LocalLog, "单频定位解算|");
    }
    strcat(LocalLog, "事后观测数据| | |");
    //获取解算结束时的时间
    time(&EndTime);
    tm_end = localtime(&EndTime);      // 转换为本地时间
    strftime(etm, sizeof(etm), "%Y-%m-%d %H:%M:%S|", tm_end);
    strcat(LocalLog, stm);
    strcat(LocalLog, etm);
    if (flag == 0)
    {
        strcat(LocalLog, " 业务执行正常\n");
    }
    else if (flag == 1)
    {
        strcat(LocalLog, " 指定设置缺少可用卫星，将切换为默认设置重新解算\n");
    }
    else if (flag == 2)
    {
        strcat(LocalLog, " 业务执行失败，缺少输入文件\n");
    }
    else
    {
        strcat(LocalLog, " 未知状态\n");
    }
...
extern int OutfilePathSet(char** infile, char* outfile, prcopt_t opt)
{
    ...
    else
    {
        ...
        strcat(temp, "/JBDH/"); // 如果是相对路径，则使用当前目录
    }
    strcpy(temp2, temp);
    fgets(temp, 100, fp);
